...
public class BlackJack {
    ...
    JFrame gameFrame = new JFrame("Black Jack");
        ...
        doubleButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                handStatusLabel.setText("Dealer's Turn");
                    ...
                    while (dealerSum < 17) {
                        Card dealCard = deck.remove(deck.size() - 1);
                        dealerSum += dealCard.getValue();
                        dealerAceCount += dealCard.isAce() ? 1 : 0;
...
public void startGame() {
        ...
        stayButton.setEnabled(false);
         ...
         commitButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                System.out.println("Text Bet: " + textBet);
                System.out.println("Bet Quant: " + betQuant);
                System.out.println("Current Bet: " + currentBet);

                if(checkZero(textBet)){
                    zeroBet = true;
                }

                else if (!checkZero(textBet)){
                    zeroBet = false;
                }

                if (textBet == null || textBet.trim().length() == 0 || textBet.matches(".*[^0-9].*") || zeroBet) {
                    betFrame.dispose();
                    errorMessage();
                }

                else {
                    betQuant = Integer.parseInt(textBet);
                    System.out.println("Bet Quant Now Is: " + betQuant);
                }

                if (betQuant > chipsPool) {
                    betFrame.dispose();
                    errorMessage();
                }
                    ...
                    currentBet = betQuant;
                    System.out.println("Current Bet Now Is: " + currentBet);
                    hitButton.setEnabled(true);
    ...
    public void errorMessage() {
        JFrame errorFrame = new JFrame("Invalid Bet");
        ...
        // Display the message in normal plain text
        JTextArea errorTextArea = new JTextArea("Invalid Bet, Please Try Again.");
    ...
    public void youLose(){
        ...
        // Display the message in normal plain text
        JTextArea lossTextArea = new JTextArea("You Lost Your Shirt...");
        lossTextArea.setEditable(false);
        lossTextArea.setWrapStyleWord(true);
        lossTextArea.setLineWrap(true);
        lossPanel.add(new JScrollPane(lossTextArea), BorderLayout.CENTER);
    
        JButton okButton = new JButton("OK");
        lossPanel.add(okButton, BorderLayout.SOUTH);
    
        lossFrame.add(lossPanel);
